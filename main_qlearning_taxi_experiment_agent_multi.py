...
configs_agent = [{
    **base_config_agent,
    "name": base_config_agent["name"] + " with action mask" if use_action_mask else base_config_agent["name"] + " without action mask",
    "use_action_mask"   : use_action_mask,
} for use_action_mask in [True, False] ]


experiment_creator = ExperimentCreator(
    savefolder=f"./plots/{base_config_env["name"]}/train/",
    savefile=f"comparison_action_mask",
    configs_env=configs_env, configs_agent=configs_agent
)

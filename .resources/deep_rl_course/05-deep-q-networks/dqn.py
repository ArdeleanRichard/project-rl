...
def dqn(env, agent, n_episodes=2000, max_t=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.995, savefile="checkpoint.pth"):
    ...
    print(n_episodes, max_t, eps_start, eps, eps_end, eps_decay)
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        print(f'\rEpisode {i_episode}\tAverage Score: {np.mean(scores_window):.2f}\tEps: {eps:.3f}', end="")
        if i_episode % 100 == 0:
            print(f'\rEpisode {i_episode}\tAverage Score: {np.mean(scores_window):.2f}\tEps: {eps:.3f}')

...
def train(policy, optimizer):
    ...
    discount_rate = .99
    epsilon = .1        # used for clipping
    beta = .01
    tmax = 320
    SGD_epoch = 4       # how many times do we use the same trajectories to update gradients
    ...
    for e in range(episode):
        ...
        for _ in range(SGD_epoch):
            ...
            L = -pong_utils.clipped_surrogate(policy, old_probs, states, actions, rewards, discount=discount_rate, epsilon=epsilon, beta=beta)
            optimizer.zero_grad()
            L.backward()
            optimizer.step()
            del L
